...
{
    char comando[200];
    //sprintf(comando, "%s %.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x", MAC_SET_DEVEUI, 0x00,0x04,0xa3,0x0b,0x00,0x1b,0x74,0x76 ); // usando no poço israel
    sprintf(comando, "%s %.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x", MAC_SET_DEVEUI, 0x00,0x04,0xa3,0x0b,0x00,0x1b,0x8f,0x98 ); 
    uint8_t status=enviaComando(comando,CMD_MAC);
...
{
    char comando[200];
    sprintf(comando, "%s %.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x", MAC_SET_APPEUI, 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00 ); 
    uint8_t status=enviaComando(comando,CMD_MAC);
    return (status==MSG_OK);
}
bool SetAppKey()
{
    char comando[200];
    //sprintf(comando, "%s %.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x", MAC_SET_APPKEY,0x55,0x55,0x55,0x55,0x55,0x55,0x55,0x55,0x55,0x55,0x55,0x55,0x55,0x55,0x55,0x55 ); // usando no poço israel 
    sprintf(comando, "%s %.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x%.2x", MAC_SET_APPKEY,0x44,0x44,0x44,0x44,0x44,0x44,0x44,0x44,0x44,0x44,0x44,0x44,0x44,0x44,0x44,0x44 ); // usando no poço israel 
    uint8_t status=enviaComando(comando,CMD_MAC);
    return (status==MSG_OK);
}
...
bool TxLoRaWan(bool conf, uint8_t porta, const uint8_t* data, uint8_t tamData)
{
    ...
    {     
        ...
        if (i == 2 || i == 3) { // nivel1
            ...
            if((retornaDado(5,0) == 1) || (retornaDado(5,1) == 1)){
                continue;}}
        else if (i == 8){// Pulsos1 2 byte
            if((retornaDado(5,0) == 1) || (retornaDado(5,1) == 1)){
                continue;}}
        else if (i == 7){// Pulsos1 1 bytes
            if((retornaDado(5,0) == 1)){
                continue;}}
            ...
            if((retornaDado(5,2) == 1) || (retornaDado(5,3) == 1)){
                continue;}}
        else if (i == 12){// Pulsos2 2 byte
            if((retornaDado(5,2) == 1) || (retornaDado(5,3) == 1)){
                continue;}}
        else if (i == 11){// Pulsos2 1 bytes
            if((retornaDado(5,2) == 1)){
                continue;}}
            ...
            if((retornaDado(5,4) == 1) || (retornaDado(5,5) == 1)){
                continue;}}
        else if (i == 16){// Pulsos3 2 byte
            if((retornaDado(5,4) == 1) || (retornaDado(5,5) == 1)){
                continue;}}
        else if (i == 15){// Pulsos3 1 bytes
            if((retornaDado(5,4) == 1)){
                continue;}}
            ...
            if((retornaDado(5,6) == 1) || (retornaDado(5,7) == 1)){
                continue;}}
        else if (i == 20){// Pulsos4 2 byte
            if((retornaDado(5,6) == 1) || (retornaDado(5,7) == 1)){
                continue;}}
        else if (i == 19){// Pulsos4 1 bytes
            if((retornaDado(5,6) == 1)){
                continue;}}
