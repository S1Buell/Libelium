...
void main(void) {
    ...
    DataRate(1);
    SetAdr(STATE_OFF);
    SetDevEUI("0004a30b001b7476"); // 0004a30b001b8f98
    SetAppEUI("0000000000000000");
    SetAppKey("55555555555555555555555555555555"); // 44444444444444444444444444444444
    MacSave();
#endif  
    uint8_t valor;
    while (true)
    {
        ...
        pisca ^= 1;
        LATEbits.LATE2 = pisca;
        
#ifdef XBEE
        dadoPoco.nivel1  =(uint16_t)(le_AD(0)*1000.0);
        dadoPoco.nivel2  =(uint16_t)(le_AD(1)*1000.0);
        dadoPoco.pulsos1 =le_pulsos(1,0);
        dadoPoco.pulsos2 =le_pulsos(2,0);
        dadoPoco.pulsos3 =le_pulsos(3,0); 
        dadoPoco.pulsos4 =le_pulsos(4,0);  
        
        createFrameHeader("poco",Sequence);
        //addSensorNivel("NIVEL",11111,11111);
        //addSensorPulso("PULSO", 1111111111, 1, 1, 1);
        if (strlen(frameBuffer) > 73)
            sendError("poco","ERROR", 1);
        else
            Sequence++;
        addSensorNivel("NIVEL",(uint16_t)(le_AD(0)*1000.0),(uint16_t)(le_AD(1)*1000.0));
        addSensorPulso("PULSO", dadoPoco.pulsos1, dadoPoco.pulsos2, dadoPoco.pulsos3, dadoPoco.pulsos4);
