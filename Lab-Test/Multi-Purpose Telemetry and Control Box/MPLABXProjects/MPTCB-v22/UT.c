...
void main(void) {
   ...
   */
    ...
    struct
    {
        //uint8_t id; FUNÇAO DESATIVADA, usando o EUI device do lora para identificação
        uint8_t feedback;
        uint16_t nivel1; 
        uint16_t nivel2;  
        uint32_t pulsos1; 
    ...
    //dadoPoco.id = ReadEEPROM(0x0069); FUNÇAO DESATIVADA, usando o EUI device do lora para identificação
...
#ifdef LORAWAN        
        //dadoPoco.numPacote++;
        /* FUNÇAO DESATIVADA, usando o EUI device do lora para identificação
